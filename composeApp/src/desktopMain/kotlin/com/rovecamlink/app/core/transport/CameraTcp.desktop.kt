 ...
 * [Dispatchers.IO]. Same code as the Android actual; the duplication beats adding a
 * shared `jvmCommon` source set (and the build-script change that comes with it) for
 * sixty lines.
 */
private class JvmCameraTcp : CameraTcp {
    override suspend fun open(host: String, port: Int, timeoutMs: Int): CameraTcpConnection =
        withContext(Dispatchers.IO) {
            ...
            // only delays the handshake, which is what flush() in the contract prevents.

...
| **H-6** | **OAuth 长期凭据写进共享存储的日志**：Google **refresh token**、Facebook access token、设备 WiFi 口令、SSID 全被 `AppLog` 打印，日志落在 `getExternalCacheDir()/BpSCam_APP_Log/`，**没有外部存储权限时退到 `Environment.getExternalStorageDirectory()/BpSCam_APP_Log/`**（整棵共享区）。refresh token = 可长期换发访问令牌的那一枚 | idGoLive | `GoogleAuthTool.java:25`、`PreviewPresenter.java:2101,2266`、`USBPreviewPresenter.java:1115`、`GraphOperation.java:31`、`BTPairCompletedFragment.java:167`（`password=[…]`）、`MWifiManager.java:42,70`；落盘路径 `Log/AppLog.java:81-85`（SDK≥29 走 app 专属外部缓存，否则整棵外部存储）+ `AppInfo.java:21` |

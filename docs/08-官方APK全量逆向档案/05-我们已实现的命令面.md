# 附录 A · 我们已实现的命令面与操作面（RoveCamLink 自研侧盘点）

> 本附录只盘点**我们自己仓库里已经实现的东西**：协议接口每个方法、每条实际发出的请求（精确到字符串）、设置项 id 全集、UI 操作入口、已知不实现的部分。
> 官方 APK 发了什么不在本文范围，见 `01`–`03` 三份档案；两侧的差距与落地顺序在 [04 · 三家操作对照与复现清单](04-三家操作对照与复现清单.md)。
...
| AMBARELLA / SIGMASTAR / TUWIN_M3 / ICATCH | 枚举存在于 `core/model/Models.kt:19-39` | 未注册 | **无协议实现**：`probe` 永远不会命中，`protocolFor()` 返回 null（`core/protocol/CameraProtocol.kt:146`） |
...
1. **idGoLive / iCatch PTP 平台完全没有插件** —— 凡是我们仓库里认得的 `ICATCH` 平台相机（含 XTU Mini1 一类），当前连接流程会止步于「没有插件认领」；
...
8. 协议接口没有定义、因而我们整体不做的：直播推流、相机侧相册编辑、多相机、云账号——接口面上就不存在（全文见 §1）。
...
| CameraProtocol 接口成员 | 20 个方法 + 4 个属性，其中 5 个带默认实现（§1） |

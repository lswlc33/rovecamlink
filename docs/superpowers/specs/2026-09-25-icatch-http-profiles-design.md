...
日期：2026-09-25 · 状态：已批准（用户确认「批准，开始执行」）
证据层：`docs/08-官方APK全量逆向档案/03-idGoLive-档案.md`（附录 A/B/C/D）与 `data/idgolive/*`
...
- **路由（纯探测，无 SSID 前缀）**：档案 §3.2 实证 idGoLive **不存在 SSID 白名单**，
  ...
  候选表（已含 `192.168.169.1` / `192.168.1.254`）+ probe 认领 —— 即 docs/03 第 12 条
  要求的「无型号、纯探测兜底路由」。

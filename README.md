...
[Releases](https://github.com/lswlc33/rovecamlink/releases) 里最新的 `nightly-*` 预发布就是 main 最新提交的产物（推 main、每日定时、手动派发都会发一条，页面只保留最近 3 条）。nightly、`test-v*` 与正式版共用同一把签名密钥，所以新包能直接覆盖升级上一个包；密钥与 `.github/apk-signer-sha256.txt` 登记不一致时 CI 会红，不会发出装了就不能再升级的包（第一次从旧的 nightly 换过来仍需先卸载一次）。

...
def main():
    ...
    for name, kind, stamp, headline, kinds in ITEMS:
        ...
        big = make_thumb(headline, stamp, name, kinds, (1280, 720) if kind == "VIDEO" else (1080, 1080))
        big.save(os.path.join(folder, name), "JPEG" if kind == "PHOTO" else "JPEG", quality=82)
        baked += 1
        print(f"  {stamp}/{name}  (+{stem}.THM)")

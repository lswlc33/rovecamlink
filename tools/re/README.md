...
字符串字面量、native 符号、布局文案。产出直接落到 `docs/evidence/data/`（生成物，勿手改），
更大的原始表留在 `_work/re/`（gitignore）。

**用途边界**：仅静态分析自有设备上安装过的官方包，用于互操作（协议/参数/取值）。产出里只记录事实与
常量，不搬运厂商源码；重实现走干净室路线，见 `docs/analysis/network-api` §0 的方法声明。
...
拿去比对语料，报出「定位不到」和「行号越界」，退出码非 0。默认扫 `docs/evidence/`。
`node tools/re/check-links.mjs [目录]` 则检查文档之间的相对链接是否都能落到真实文件（并行写作时
出现过「目录里列了三个还没写的附录」）。

`node tools/re/check-refs.mjs` 是文档系统的守卫：解析全仓（docs 正文、代码注释、工具、官网）每一处
`docs/<层>/<slug>` 引用与紧跟其后的 `§ 小节号`，并检查每篇文档在所属层 README 登记过。CI 的
`docs-check` 工作流跑它 + check-links + check-tables；check-citations 因依赖本地语料只在本地跑。

`node tools/re/check-tables.mjs [路径…]` 检查每张表格的列数是否与表头一致（GFM 表格先按 `|` 切单元格，
再做行内解析，所以代码段里的裸 `|` 会把一行劈成多列）；带 `--fix` 会自动把**反引号内**的竖线转义成 `\|`，
剩下的（真缺列/多列）需要人判断，别让它自动改。

`oneoffs/` 里是已执行完的一次性迁移脚本（附录归并、旧编号 → 四层目录重整），留作记录，别再跑。
